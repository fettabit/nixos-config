...
- **Panels** (one PR each): network + bluetooth panel — now via native `Quickshell.Networking`/`Quickshell.Bluetooth` (no nmcli/bluetoothctl parsing); calendar (+weather); music popup + cava; monitors; settings; guide.
