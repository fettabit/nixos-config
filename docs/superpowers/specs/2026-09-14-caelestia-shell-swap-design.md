...
**Date:** 2026-09-14 · **Status:** approved by jftx (design walkthrough), pending external second opinion (Codex) · **Issue:** #24 · **Branch:** `feat/caelestia-shell` · **Supersedes:** `docs/plans/quickshell-matugen-migration.md`, epic #7 (Track C), #17 (calendar/weather section)
...
- **Drawers.** Launcher (fuzzy desktop-entry search; `>` prefix → actions:
  `calc`, `scheme`, `variant`, `wallpaper` (grid), `light`, `dark`, `lock`,
  `logout`, `reboot`, `settings`; `shutdown`/`sleep`/`random` exist on `main`
  only; dangerous ones gated by `launcher.enableDangerousActions`, default
  off). Sidebar = notification centre. Dashboard = date/time, calendar,
...
in {
  ...
  home.packages = [ pkgs.pwvucontrol ];      # general.apps.audio default; bar audio popout hands off to it
...
without it), `pkgs.pwvucontrol`, `pkgs.papirus-icon-theme` (via
`gtk.iconTheme.package`). Nothing else: the two caelestia packages already
carry every external tool they call.
...
settings = {
  ...
  general.apps = {
    ...
    # audio = [ "pwvucontrol" ]  (default; package added in 3.1)
  };
...
wanted), `launcher.enableDangerousActions = false`, `launcher.actions` (stock),
...
| GTK 3/4 | HM `gtk3/gtk4.extraCss` `@import` + `dconf.settings` + gsettings bounce | CLI atomically writes `gtk-3.0/gtk.css`, `gtk-4.0/gtk.css` and sets dconf `gtk-theme`/`color-scheme`/`icon-theme` per apply. HM stops owning those: drop `extraCss` (else HM symlink vs CLI file fight every `rb`), drop `dconf.settings` (HM would reset keys on activation), drop `gtk-application-prefer-dark-theme` (light mode must be allowed). Keep `gtk.enable`, `theme = adw-gtk3` (caelestia hardcodes `adw-gtk3-dark` as the GTK theme name in both modes and recolours via gtk.css). | `theme.nix` |
| Icons | Adwaita | `gtk.iconTheme = Papirus-Dark` (initial `settings.ini`; package `papirus-icon-theme`); CLI flips `Papirus-Dark/Light` with mode via dconf. No folder recolouring (D10). | `theme.nix` |
...
{ pkgs, ... }: {
  gtk = {
    enable = true;
    theme = { name = "adw-gtk3-dark"; package = pkgs.adw-gtk3; };   # caelestia keeps this name in both modes
    iconTheme = { name = "Papirus-Dark"; package = pkgs.papirus-icon-theme; };
    # gtk.css and the dconf keys are written by caelestia-cli on every scheme apply — HM must not own them.
  };
  qt = {
    enable = true;
    platformTheme = { name = "qtengine"; package = pkgs.qtengine; };
  };
  home.packages = [ pkgs.darkly ];   # Qt6 style named in caelestia's qtengine config
}
```
...
| `modules/home/desktop/hypr/modules/{binds,autostart,env,decorations,windowrules}.lua` | per 3.4 |
| `flake.nix` | +input |
...
5. **Bootstrap the scheme:** `caelestia wallpaper -f ~/wallpapers/moon.jpg`.
   ...
   `sequences.txt`; `~/.config/hypr/scheme/current.lua` exists and window
...
8. **Lock, with an escape hatch:** `Ctrl+Alt+F2`, log in on TTY2, back to
   TTY1, then ALT+L. If PAM refuses the password: TTY2 → `systemctl --user
   restart caelestia`.
...
2. **`hyprctl reload` from the service environment** — needs
   `HYPRLAND_INSTANCE_SIGNATURE`; the hook's runtime-dir discovery covers its
   absence. If Hyprland already auto-reloads on `dofile`d file changes, it is a
...
6. **2.3.0 vs `main` drift** — every key and shortcut used here was grepped in
   the 2.3.0 tarball; `>random`/`>shutdown`/`>sleep` launcher actions are
   `main`-only and not relied on (ALT+W is the random path).
...
- Bumping to caelestia 2.4.0 (`src` override or wait for nixpkgs; `>random`
  arrives with it).
